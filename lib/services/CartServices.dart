...
class CartServices{
  ...
  static addCart(item) async{
    ...
    try {
      ...
      var cartList = await Storage.getString('cartList');

      //没有数据的时候,直接存储
      if (cartList == null) {
        List tempList = [];
        tempList.add(item);
        await Storage.setString(cartKey, json.encode(tempList));
        return;
      }
